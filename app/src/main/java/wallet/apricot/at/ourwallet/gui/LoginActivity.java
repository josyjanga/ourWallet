  ...
  public class UserLoginTask extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... params) {
      ...
      try {
        ...
        //Thread.sleep(2000);
        p = new HashMap<>();
        p.put("username", mEmail);
        p.put("password", mPassword);

        int bugCounter=0;
        Object result=ParseCloud.callFunction("loginWithAPI", p);
        // workaround for API bug
        while(result==null && bugCounter < 3){
          Thread.sleep(200);
          result=ParseCloud.callFunction("loginWithAPI", p);
          bugCounter++;
        }
    ...
    protected void onPostExecute(final String id) {
      ...
      if (!id.equals("")) {
        Util.storeStringInPref(getApplicationContext(),AppConstants.USER_ID,id);
        // get User data from Parse & store locally afterwards
        ParseQuery<Member> query = ParseQuery.getQuery("Member");
        query.getInBackground(id, new GetCallback<Member>() {
          public void done(Member object, ParseException e) {
            if (e == null) {
              // object will be your game score
              EntityManager.INSTANCE.saveMyUser((Member) object);
              finish();

            } else {
              // something went wrong
              Toast.makeText(LoginActivity.this, "User couldn't be loaded!", Toast.LENGTH_SHORT).show();
            }
          }
        });
      } else {
